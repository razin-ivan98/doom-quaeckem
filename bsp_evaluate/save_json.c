...
void	check_walls(t_bsp *node, int i, int fd)
{
	...
	ft_putstr_fd("{\n\t\t\t\"points\": [\n\t\t\t\t[", fd);
	str = ftoa(node->walls[i].points[0].x, 1, str);
	ft_putstr_fd(str, fd);
	ft_putstr_fd(", ", fd);
	str = ftoa(node->walls[i].points[0].y, 1, str);
	ft_putstr_fd(str, fd);
	ft_putstr_fd("],\n\t\t\t\t[", fd);
	str = ftoa(node->walls[i].points[1].x, 1, str);
	ft_putstr_fd(str, fd);
	ft_putstr_fd(", ", fd);
	str = ftoa(node->walls[i].points[1].y, 1, str);
	ft_putstr_fd(str, fd);
	ft_putstr_fd("]\n\t\t\t],\n\t\t\t\"normal\": [", fd);
...
void	check_not_leaf(t_bsp *node, int fd)
{
	...
	ft_putstr_fd(str, fd);
	ft_putstr_fd(",\n\t\"line\": [", fd);
	str = ftoa(node->line.x, 1, str);
	ft_putstr_fd(str, fd);
	ft_putstr_fd(", ", fd);
	str = ftoa(node->line.y, 1, str);
	ft_putstr_fd(str, fd);
	ft_putstr_fd("],\n\t\"normal\": [", fd);
...
void	save_json(t_bsp *root, int not_leaves)
{
	...
	ft_putstr_fd("{\n", fd);
	bsp_json_save_traversal(root, 0, fd, not_leaves);
	while (i < not_leaves)
	{
		ft_putstr_fd("}\n", fd);
		i++;
