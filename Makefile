...
FTPATH = ./libft
FRAMEWORKS = -F./frameworks -rpath ./frameworks -framework SDL2 -framework SDL2_ttf\
...
ifeq ($(UNAME_S), Darwin)
	@echo MAC OS
	make -C $(FTPATH)
	gcc -g $(SRCS) -fsanitize=address -lm -L./libft -lft $(FRAMEWORKS) $(INCLUDES) -o DoomQuaeckem
else
	@echo Linux
	make -C $(FTPATH)
