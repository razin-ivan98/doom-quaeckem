...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/15 01:04:56 by Chorange          #+#    #+#             */
/*   Updated: 2021/01/15 01:04:56 by Chorange         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_in_figure(t_vertex *pts, int pts_count, int i)
{
	...
	base = normalize((t_vertex){
		...
		pts[get_i_minus_1(i, pts_count)].y - pts[i].y, 0.0});
	new = normalize((t_vertex){
		pts[get_i_plus_2(i, pts_count)].x - pts[i].x,
		pts[get_i_plus_2(i, pts_count)].y - pts[i].y, 0.0});
	end = normalize((t_vertex){
		pts[get_i_plus_1(i, pts_count)].x - pts[i].x,
		pts[get_i_plus_1(i, pts_count)].y - pts[i].y, 0.0});
...
int add_n(t_map *map, t_vertex n)
{
	...
	{
		if (map->nrmls[i].x == n.x && map->nrmls[i].y ==
							n.y && map->nrmls[i].z == n.z)
			return (i);
		i++;
	}
...
int add_vt(t_map *map, t_vertex vt)
{
	...
	{
		if (map->vts[i].x == vt.x && map->vts[i].y ==
							vt.y && map->vts[i].z == vt.z)
			return (i);
		i++;
	}
...
void	floor_tr_vts(t_bsp *node, t_map *map, t_vt_tr *new, int i)
{
	t_vertex vt;
...
void	floor_tr(t_bsp *node, t_map *map, t_vt_tr *new, int i)
{
	...
	floor_tr_vts(node, map, new, i);

	vt = triangle_normal(map->vts[new->ids[0]], map->vts[new->ids[1]],
									map->vts[new->ids[2]]);
	new->n_ids[0] = add_n(map, vt);
	new->n_ids[1] = new->n_ids[0];
	new->n_ids[2] = new->n_ids[0];
	uv = (t_vertex){node->trs[i].points[0].x * TEXTURE_SCALE,
				node->trs[i].points[0].y * TEXTURE_SCALE, 0.0};
	new->uv_ids[0] = add_uv(map, uv);
	uv = (t_vertex){node->trs[i].points[1].x * TEXTURE_SCALE,
				node->trs[i].points[1].y * TEXTURE_SCALE, 0.0};
	new->uv_ids[1] = add_uv(map, uv);
	uv = (t_vertex){node->trs[i].points[2].x * TEXTURE_SCALE,
...
void	to_obj_format(t_bsp *node, t_map *map)
{
	...
	i = 0;
	if (!(node->vt_trs = malloc(sizeof(t_vt_tr) * 30000)))
		exit(-2);
	node->vt_trs_count = 0;
	while (i < node->trs_count)
	{
		floor_tr(node, map, &new, i);
		ceil_tr(node, map, &new, i);
		i++;
	}
	i = 0;
	while ( i < node->walls_count)
...
static void	write_vt_trs(t_bsp *node, int fd, int i)
{
	ft_putstr_fd("f ", fd);
...
void		write_vt_trs_to_file(t_bsp *node, int fd)
{
	...
	{
		write_vt_trs(node, fd, i);
		i++;
	}
	ft_putendl_fd("", fd);
}

void	write_to_file_uvs(t_map *map, int fd, char *ptr)
{
	int		i;
...
void	write_to_file_nrmls(t_map *map, int fd, char *ptr)
{
	int		i;
...
void	write_to_file_vs(t_map *map, int fd, char *ptr)
{
	int		i;
...
void	write_to_file(t_map *map, int fd)
{
	...
	if (!(ptr = malloc(256)))
		exit(-2);
	write_to_file_uvs(map, fd, ptr);
	write_to_file_nrmls(map, fd, ptr);
	write_to_file_vs(map, fd, ptr);
...
void	export_map_2(t_map_editor *ed, char *str, int fd)
{
