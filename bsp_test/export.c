...
void to_obj_format(t_bsp *node, t_map *map)
{
	...
	while (i < node->trs_count)
	{
		...
		uv = (t_vertex){0.0, 1.0, 0.0};
		new.uv_ids[0] = add_uv(map, uv);
		uv = (t_vertex){0.0, 0.0, 0.0};
...
void	write_vt_trs_to_file(t_bsp *node, t_map *map, int fd)
{
	int i;
...
void	bsp_export_traversal(t_bsp *node, t_map *map, int fd)
{
	t_vertex	pts[100];
	int			pts_count;
	...
	if (node->is_leaf)
	{
		...
		write_vt_trs_to_file(node, map, fd);
		return ;
	}
	else
	{
		bsp_export_traversal(node->front, map, fd);
		bsp_export_traversal(node->back, map, fd);
	}
}
...
void	export_map(t_map_editor *ed)
{
	...
	fd = open("levl.obj", O_RDWR | O_TRUNC);
	write_to_file(&ed->map, fd);
	close(fd);

	fd = open("levl.obj", O_RDWR | O_APPEND);
	ft_putendl_fd("", fd);
